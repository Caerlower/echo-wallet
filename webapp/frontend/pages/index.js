...
export default function Home() {
  ...
  const loadWalletData = async (walletAddress) => {
    setLoading(true);
    try {
      // Load portfolio using the backend route
      const portfolioResponse = await fetch(`${API_BASE}/wallet/portfolio/${walletAddress}`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Left Sidebar - Suggestions and Monitoring */}
          <div className="lg:col-span-1 space-y-6">
            ...
            {/* Monitoring Panel */}
            {isConnected && address && (
              <MonitoringPanel walletAddress={address} />
            )}
          </div>
